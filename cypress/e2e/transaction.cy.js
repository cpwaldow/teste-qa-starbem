describe('Transição aprovadas entre contas', () => {
  ...
  it('Fazer login com sucesso - transação maior que o valor disponível', () => {
    const userEmail = 'maisDinheiro@bugbank.com';
    const userPassword = 'maisDinheiro';
    const userName = 'Belo';
    ...
    cy.get(
      ...
      .type(1001);
    cy.contains('Transferir agora').click();
    cy.contains('Você não tem saldo suficiente para essa transação').should(
      'be.visible',
    );
    cy.contains('Fechar').click();
    cy.contains('Voltar').click();
  ...
  it('Verificar se o valor caiu de forma correta', () => {
    const userEmail = 'testeTransacao@bugbank.com';
    const userPassword = 'senhaTransacao';
    const userName = 'Thiago Maia';
    ...
    cy.getAllLocalStorage().then((result) => {
      const assistantAccount =
        result['https://bugbank.netlify.app']['contaAuxiliar@bugbank.com'];
      const { email, password, name } = JSON.parse(assistantAccount);
      cy.get('input[type="email"]').first().focus().type(email);
      cy.get('input[type="password"]').first().focus().type(password);
      cy.contains('Acessar').first().click();
      cy.contains(`Olá ${name}`);
      cy.contains('Saldo em conta R$ 500,00').should('be.visible');
    });
  });
});

describe.skip('Transação negada entre contas', () => {
  beforeEach(() => {
    cy.visit('https://bugbank.netlify.app/');
  });
  ...
  it('Valor de transferência não pode ser igual a zero', () => {
    const userEmail = 'transferenciaInvalida@bugbank.com';
    const userPassword = 'transferenciaInvalida';
    const userName = 'Lorena Magalhães';
    ...
    cy.get(
      ...
      .type(0);
    cy.contains('Transferir agora').click();
    cy.contains('Valor da transferência não pode ser 0 ou negativo').should(
      'be.visible',
    );
    cy.contains('Fechar').click();
    cy.contains('Voltar').click();
  ...
  it('Valor de transferência não pode ser menor que zero', () => {
    const userEmail = 'transferenciaInvalida@bugbank.com';
    const userPassword = 'transferenciaInvalida';
    const userName = 'Flávio Autobots';
